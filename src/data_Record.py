...
class DataRecordUI(QDialog, QWidget):
    receiveLogSignal = pyqtSignal(str)

    def __init__(self):
        super(DataRecordUI, self).__init__()
        loadUi('./src/DataRecord.ui', self) ##使用加载可以不用PyUIC来进行转化
        #self.setWindowIcon(QIcon('./icons/icon.png'))
        self.setFixedSize(1011, 601)

        # 第一人脸是否读入的标记
        self.first_faceimage = False
        ...
        self.initDbButton.clicked.connect(self.initDb) #建立db和表格
        ...
        self.minFaceRecordCount = 2 #至少采集的张数
    ...
    def startWebcam(self, status):
        if status:
            ...
            if not ret:
                ...
                self.timer.start(5) ##同时开始更新frame
        ...
        if startFaceRecordButton.text() == '开始采集人脸数据':
            if self.isFaceDetectEnabled:
                if self.isUserInfoReady:
                    self.addOrUpdateUserInfoButton.setEnabled(False)##当前信息可用就禁止添加或修改
    ...
    def detectFace(self, frame):
        ...
        locations = face_locations(small_frame,
                                   model='cnn',
                                   number_of_times_to_upsample=1) #face_recognition模块中调用人脸定位
        ...
        for (top, right, bottom, left) in locations:
            top, right, bottom, left = (x * 4 for x in (top, right, bottom, left))  # 坐标变回原图的规格
            ...
            if self.isFaceRecordEnabled:
                try:
                    ...
                    cv2.imwrite('{}/stu_{}/img.{}.jpg'.format(self.datasets, stu_id, self.faceRecordCount + 1),
                                frame[top - 20:bottom + 20, left - 20:right + 20]) #frame[y - 20:y + h + 20, x - 20:x + w + 20]
                    if self.first_faceimage == False:
                        self.first_faceimage = True
                        fistimage_path = QFileDialog.getExistingDirectory(self, "选取保存的文件夹", "./pic",
                                                                           options=QFileDialog.DontUseNativeDialog)
                        if fistimage_path =='':
                            continue
                        files = os.listdir(fistimage_path)
                        if (cn_name+'.jpg') not in files:  #第一张照片不存在则写入相应路径
                            cv2.imwrite('{}/{}.jpg'.format(fistimage_path, cn_name),
                                        frame)
                            # 生成人脸向量
                            ...
                            cursor.execute('SELECT cn_name FROM users WHERE cn_name=?', (cn_name,))
                            result = cursor.fetchone()
                            if result == None:
                                #cursor.execute('INSERT INTO users (stu_id, cn_name, en_name) VALUES (?, ?, ?)',
                                #   (stu_id, cn_name, en_name,))
                                cursor.execute('INSERT INTO users (stu_id, cn_name, en_name, encoding) VALUES (?, ?, ?, ?)',
                                                  (stu_id, cn_name, en_name, encodings[0], ))
                            cursor.close()
                            conn.commit()
                            conn.close()
            ...
            #cv2.rectangle(frame, (left - 5, bottom - 10), (right + 5, top + 10), (0, 0, 255), 2)##画框
    ...
    def displayImage(self, img):
        ...
        self.faceDetectCaptureLabel.setScaledContents(True) ##扩展pixmap到整个空间
    ...
    def addOrUpdateUserInfo(self):
        ...
        stu_id, cn_name, en_name = self.userInfo.get('stu_id'), self.userInfo.get('cn_name'), self.userInfo.get(
            'en_name')
        self.userInfoDialog.stuIDLineEdit.setText(stu_id) ##输入
            ...
            try:
                ...
                if cursor.fetchall():
                    ...
                    ret = DataRecordUI.callDialog(QMessageBox.Warning, text, informativeText,
                                                  QMessageBox.Yes | QMessageBox.No) ##static通过类对象直接调用，注意staticmethod和classmethod和property区别
    ...
    def callDialog(icon, text, informativeText, standardButtons, defaultButton=None):
        ...
        msg.setStandardButtons(standardButtons) ##设置yes or no
        if defaultButton:
...
class UserInfoDialog(QDialog):
    def __init__(self):
        ...
        loadUi('./src/UserInfoDialog.ui', self)
        #self.setWindowIcon(QIcon('./icons/icon.png'))
